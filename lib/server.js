...
if(!args['server-conf']) {
    ...
    args['server-conf'] = __dirname+"/../conf/server.conf.default";
}
logger.info('reading configServer from ' + args['server-conf']);
mod_fs.readFile(args['server-conf'], function (err, data) {
    ...
    logger.info('configServer loaded');
    if(configModules)
...
if(!args['module-conf']) {
  ...
  args['module-conf'] = __dirname+"/../conf/module.conf.default";
}
logger.info('reading configModules from ' + args['module-conf']);
mod_fs.readFile(args['module-conf'], function (err, data) {
  if (err) {
      logger.error('error reading module configuration');
      process.exit();
  }
  
  configModules = JSON.parse(data);
  logger.info('configModules loaded');
  if(configServer)
    configureServer(configServer, configModules);

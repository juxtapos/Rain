...
function handleViewRequest (req, res, next) {
	...
	r.render('file:///Users/cag/workspace/rain/modules/app/htdocs/index.html').then(function (doc) {
		console.timeEnd('render');
		res.end(doc);
		
	});

	// view(req.params[0], req.params[1], mode, baseurl).then(function (data) {
	// 	//logger.debug(mod_sys.inspect(data.toString()))
	// 	res.end(data.output);
	// })
}

function view(modulename, viewname, mode, baseurl) {
	//logger.debug('view        ' + modulename)
	var promise = new mod_promise.Promise();
	var moduleUrl = 'file://' + mod_path.join(moduleRootFolder, modulename, 'htdocs', viewname);
	var renderer = new mod_renderer.Renderer();
	mod_resourcemanager.getResource(moduleUrl).then(function (resource) {
		var content = resource.data.toString();
	 	renderer.render(baseurl, content, tagmanager, mode, view).then(function (content) {
	 		logger.debug('rendering complete '/* + mod_sys.inspect(content)*/);
	 		promise.resolve(mod_sys.inspect(content, true));
	 	});
	});
	return promise;
}

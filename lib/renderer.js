	...
	return this._render(url).then(function (renderres) {
       c(self.renderResources.length);
        var deps = self.calcDependencies(renderres)
            , out = self[mode!=='json' ? 'outputHtml' : 'outputJson'](renderres, deps.css, deps.scripts);
        return out;
    } );
}
...
Renderer.prototype.outputHtml = function (resource, css, scripts) {
    ...
    if (resource.elements.length > 0) {
        ...
        resource.elements.forEach(function (elem) {
            ...
            markup.push('\nrequire(["/modules/', elem.moduleId, '/client.js", "text!/modules/'
                          , elem.moduleId,'/main.html?type=json"', im
                          , '], function (module, template, instance) { module.initView("'
                          , elem.id, '", template, instance) } );'
            );
        });
        markup.push('\n</script>\n');
    }
...
Renderer.prototype.outputJson = function (resource, css, scripts) {

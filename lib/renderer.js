...
//        data.city = 'Test' + new Date().getTime();
        ...
        // 3. Do translation
        this.translationRenderer(tmplText, function(template){
          // 4. Do templating
          buffer = "";
    ...
    Renderer.prototype.translationRenderer = function(template, callback){
      var cleanpath = mod_path.join(this.modulepath+"/htdocs/locales/"+language+".po"),
          self = this;
      gettext.setlocale('LC_ALL', language);
      gettext.loadLanguageFile(mod_path.join(documentRoot, cleanpath), language, function(){
        logger.debug('translationRenderer ' + self.uuid);
        //title tag as exception
        template = template.replace(/<title>\s*gettext\(\s*['"](.*)['"]\s*\)\s*<\/title>/g, function(a, b, c, d){
          return '<title class="gettext" data-msgid="'+b+'">'+gettext.gettext(b)+'</title>';
        });
        template = template.replace(/<title>\s*ngettext\(\s*['"](.*)['"]\s*,\s*['"](.*)['"]\s*,\s*(\d*)\s*\)\s*<\/title>/g, function(a, b, c, d){
          return '<title class="ngettext" data-msgid="'+b+'" data-msgid_plural="'+c+'" data-n="'+d+'">'+gettext.ngettext(b, c, parseInt(d))+'</title>';
        });
        //title tag ends
        
        
        //all other translations
        template = template.replace(/gettext\(\s*['"](.*)['"]\s*\)/g, function(a, b, c, d){
          return '<span class="gettext" data-msgid="'+b+'">'+gettext.gettext(b)+'</span>';
        });
        template = template.replace(/ngettext\(\s*['"](.*)['"]\s*,\s*['"](.*)['"]\s*,\s*(\d*)\s*\)/g, function(a, b, c, d){
          return '<span class="ngettext" data-msgid="'+b+'" data-msgid_plural="'+c+'" data-n="'+d+'">'+gettext.ngettext(b, c, parseInt(d))+'</span>';
        });
        callback(template);
      });
    };

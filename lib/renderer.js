...
//        data.city = 'Test' + new Date().getTime();
        ...
        // 3. Do templating
        mod_mu.compileText('preTemplate', tmplText);
        var stream = mod_mu.render('preTemplate', data)
          .on('data', function (c) {
              buffer += c;
          })
          .on('end' , function () {
              // 4. Do translation
              self.translationRenderer(buffer, function(template){
                // 5. Do templating again for the new mustache tags of the translations
                buffer = "";
                mod_mu.compileText('postTemplate', template);
                  stream = mod_mu.render('postTemplate', data)
                  .on('data', function (c) {
                    buffer += c;
                  })
                  .on('end' , function () {
/* if a translation in a translation is needed
                    self.translationRenderer(buffer, function(template){
                      self.preRenderResult = template; */
    ...
    Renderer.prototype.translationRenderer = function(template, callback){
      var language = 'de_DE',
          cleanpath = mod_path.join(this.modulepath+"/htdocs/locales/"+language+".po"),
          self = this;

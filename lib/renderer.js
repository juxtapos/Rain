...
Renderer.prototype.render = function (url, doc, tagmanager, mode) {
    var promise = new mod_promise.Promise()
        , self = this;
    this.parse(url, doc, tagmanager, mode).then(function (d) {
        var c = self.renderDependencies(d.doc,d.outputBuffer,d.elements,d.resources, mode)
        promise.resolve(c);
    });
    return promise;
}

Renderer.prototype.push = function (url) {
    logger.debug('psuh: ' + url);
    this._resources.push();
}

Renderer.prototype.parse = function (url, doc, tagmanager, mode) {
    var defer = new mod_promise.Promise()
        , parser = new xml.SaxParser(new DocParser(defer, this.renderDependencies)),
        self = this;
      ...
      cb.onStartElementNS(function(elem, attrs, prefix, uri, namespaces) {
          ...
          if (elem === 'link') {
            ...
            if (tag !== null) {
              ...
              self.push('adding to the render thing!');

  ...
  return function errorHandler(err, req, res, next){
    ...
    if (showStack) {
      ...
      if (~accept.indexOf('html')) {
        fs.readFile(mod_path.join(Server.conf.server.serverRoot, '/public/style.css'), 'utf8', function(e, style){
          fs.readFile(mod_path.join(Server.conf.server.serverRoot, '/public/error.html'), 'utf8', function(e, html){
            var stack = (err.stack || '')
              .split('\n').slice(1)

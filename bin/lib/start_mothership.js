...
var ms_server_conf = fs.readFileSync(
  ...
  , 'utf8'),
   mpid_path = mod_path.join(__dirname, '../..', '.process', 'm_pid')
  ,actPath   = process.cwd()
...
if(utils.mothershipIsUp(parsedConf.server.port)){
  process.stdout.write("mothership started");
  process.exit(0);
}
...
ms_server(JSON.parse(ms_server_conf), function(){
  ...
  daemon.daemonize('/dev/null', null, function(err, pid){
    ...
    var conf_mpid    = mod_path.join(mpid_path, "server.conf."+parsedConf.server.port+"."+pid);
    //write server config
